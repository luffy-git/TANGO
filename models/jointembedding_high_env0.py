...
class MLP(nn.Module):
    def __init__(self, in_dim, hidden_size, out_dim):
        super().__init__()
        self.mlp = nn.Sequential(nn.Linear(in_dim, hidden_size), nn.LeakyReLU(0.2, True), nn.Linear(hidden_size, out_dim))
...
class WrapedMotionCNN(nn.Module):
    def __init__(self, args):
        ...
        self.encoder_trans = nn.TransformerEncoder(
            encoder_layer, num_layers=1
        )  # Wav2Vec2Model.from_pretrained('facebook/wav2vec2-base-960h').encoder
    ...
    def forward(
        ...
        return {"low_level": low_level, "high_level": hidden_states}
...
class JointEmbedding(nn.Module):
    def __init__(self, args):
        ...
        self.audio_sa = CustomMultiheadAttention(
            embed_dim=self.args.audio_f,
            num_heads=8,
        )
        ...
        self.motion_sa = CustomMultiheadAttention(
            embed_dim=self.args.audio_f,
            num_heads=8,
        )
    ...
    def forward(self, in_audio=None, in_motion=None, cached_audio_low=None, cached_audio_high=None, cached_rep15d=None):
        ...
        motion_high_att_before_sum = F.interpolate(
            motion_high_att_before_sum.transpose(1, 2), scale_factor=2, mode="linear", align_corners=True
        ).transpose(1, 2)
        ...
        if cached_audio_low is not None:
            ...
            raw_audio_high = torch.cat(
                [self.audio_down_proj_2(cached_audio_high[:, :, :768]), self.audio_down_proj_3(cached_audio_high[:, :, 768:])], dim=-1
            )
        ...
        finetune_audio_high = F.interpolate(
            finetune_audio_high.transpose(1, 2), scale_factor=15 / 50, mode="linear", align_corners=True
        ).transpose(1, 2)
        ...
        audio_high_att_before_sum = F.interpolate(
            audio_high_att_before_sum.transpose(1, 2), scale_factor=2, mode="linear", align_corners=True
        ).transpose(1, 2)
    ...
    def get_audio_features(self, in_audio):
        ...
        _, bert_time_aligned_text, _ = audio_to_time_aligned_text_features(
            inputs, self.audio_processor, self.asr, self.bert_tokenizer, self.bert_model
        )
        raw_audio_high = torch.cat([raw_audio_high, bert_time_aligned_text], dim=2)
        raw_audio_high = torch.cat(
            [self.audio_down_proj_2(raw_audio_high[:, :, :768]), self.audio_down_proj_3(raw_audio_high[:, :, 768:])], dim=-1
        )
